...
@test "optional pattern is cloned" {
  ...
  patterns=($(curl -s --noproxy localhost http://localhost:8500/v1/kv/cloudconductor/parameters?raw\&$token={token} | jq -r '.cloudconductor.patterns | .[] | select(.type == "optional") | .name'))
